...
import services.wiring.rf.RfAssemblyService
import wiring.rf.RfAssemblyParam
...
object RfAssemblyController:
  given entityDecoder[
    F[_]: effect.Concurrent,
    RfConnectorID: Decoder,
    CategoryID: Decoder,
    TagID: Decoder,
  ]: EntityDecoder[F, RfAssemblyParam[RfConnectorID, CategoryID, TagID]] =
    jsonOf
  ...
  def make[
    ...
    RfConnectorService: RfAssemblyService[F, RfAssemblyID, RfConnectorID, CategoryID, TagID]
  )(using
    parse: helpers.Parse[String, RfAssemblyID]
  ): Controller[F] =
    new Controller[F] with Http4sDsl[F]:
      private val prefixPath: String = "/rf/rfassembly"
      ...
      private val httpRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
        ...
        // POST ROUTES
        case r @ POST -> Root =>
          r.as[RfAssemblyParam[RfConnectorID, CategoryID, TagID]]
            .flatMap(x => create(x))
      ...
      /** Helper method to for creating a Tag from a POST request
        ...
        */
      private def create(input: RfAssemblyParam[RfConnectorID, CategoryID, TagID]): F[Response[F]] =
        RfConnectorService
          .create(
            input
          )
      ...
      private def parseID(
        ...
        success: RfAssemblyID => F[Response[F]]
      ): F[Response[F]] =
        parse(id).fold(
