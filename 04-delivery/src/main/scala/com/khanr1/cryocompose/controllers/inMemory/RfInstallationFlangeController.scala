...
object RfInstallationFlangeController:
  given entityDecoder[
    F[_]: effect.Concurrent,
    RfConnectorID: Decoder,
    CategoryID: Decoder,
    TagID: Decoder,
  ]: EntityDecoder[F, RfInstallationFlangeParam[RfConnectorID, CategoryID, TagID]] =
    jsonOf
  ...
  def make[
    ...
    RfInstallationFlangeService: RfInstallationFlangeService[
      F,
      RfConnectorID,
      FlangeID,
      CategoryID,
    ...
    new Controller[F] with Http4sDsl[F]:
      private val prefixPath: String = "/rf/rfflange"
      ...
      private val httpRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
        ...
        // POST ROUTES
        case r @ POST -> Root =>
          r.as[RfInstallationFlangeParam[RfConnectorID, CategoryID, TagID]]
            .flatMap(x => create(x))
      ...
      /** Helper method to for creating a Tag from a POST request
        ...
        */
      private def create(input: RfInstallationFlangeParam[RfConnectorID, CategoryID, TagID])
        : F[Response[F]] =
        RfInstallationFlangeService
          .create(
            input
          )
          .map(_.asJson)
      ...
      /** helper method to search all tags
        */
      def searchAll: F[Response[F]] = RfInstallationFlangeService
        .findAllRfInstallationFlange
        .flatMap { tags =>
          tags
            .asJson
            .pipe(Ok(_))
