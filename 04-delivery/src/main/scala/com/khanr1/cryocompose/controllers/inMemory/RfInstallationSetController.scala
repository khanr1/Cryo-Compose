...
object RfInstallationSetController:
  given entityDecoder[
    ...
    CategoryID: Decoder,
    TagID: Decoder,
  ]: EntityDecoder[F, RfInstallationSetParam[RfConnectorID, FlangeID, CategoryID, TagID]] =
    jsonOf

  def make[
    F[_]: effect.Async,
    RfConnectorID: Encoder: Decoder,
    FlangeID: Encoder: Decoder,
    CategoryID: Encoder: Decoder,
    TagID: Encoder: Decoder,
  ](
    RfInstallationSetService: RfInstallationSetService[
      F,
      RfConnectorID,
      FlangeID,
      CategoryID,
    ...
    new Controller[F] with Http4sDsl[F]:
      private val prefixPath: String = "/rf/rfflangeset"

      private val httpRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
        // GET ROUTES
        case GET -> Root => searchAll
        // POST ROUTES
        case r @ POST -> Root =>
          r.as[RfInstallationSetParam[RfConnectorID, FlangeID, CategoryID, TagID]]
            .flatMap(x => create(x))
      ...
      /** Helper method to for creating a Tag from a POST request
        ...
        */
      private def create(input: RfInstallationSetParam[RfConnectorID, FlangeID, CategoryID, TagID])
        : F[Response[F]] =
        RfInstallationSetService
          .create(
            input
          )
          .map(_.asJson)
      ...
      /** helper method to search all tags
        */
      def searchAll: F[Response[F]] = RfInstallationSetService
        .findAllRfInstallationSet
        .flatMap { tags =>
          tags
            .asJson
            .pipe(Ok(_))
